...
- Добавлена кнопка `Настройки`: GUI показывает текущий глобальный config devctl, позволяет выбрать путь Patch Inbox и выполнить регистрацию текущего workspace без терминала.
- Убраны непрозрачные фоны у текстовых QLabel внутри карточек/блоков, поэтому исчезают чёрные полосы вокруг заголовков и значений.
- Панель быстрых действий теперь использует стандартные Qt `QToolButton` + `QStyle` icons вместо набора текстовых Unicode-глифов. Это выглядит нативнее в flat-теме и лучше масштабируется.
- `build_exe.ps1` устанавливает PySide6, делает smoke-import, собирает PyInstaller и проверяет готовый `.exe` через child-mode.

## Новое в v0.5.1

- Уточнена рекомендация по умолчанию: если структура workspace уже актуальна, Git чистый и новых патчей нет, GUI показывает спокойное состояние «Workspace готов, новых патчей нет», а не жёлтое «Обновить структуру workspace».
- Необязательные недостающие `archive.exclude` считаются maintenance-подсказкой и не блокируют нормальный idle-state.
- Карточка «Патч» больше не становится жёлтой, когда patch.zip просто отсутствуют: это нормальное состояние для пустой очереди.
- Панель быстрых действий получила собственные минималистичные flat vector icons, рисуемые через QPainter. Это убирает зависимость от системных иконок в стиле 90-х и делает кнопку запуска заметной зелёной flat-кнопкой.

## Новое в v0.5.2

- Версия GUI поднята до `0.5.2` без изменения core-версии devctl.
- Toolbar больше не рисует пиктограммы процедурно через `QPainter`: вместо этого используются локальные плоские SVG-файлы из `gui/assets/icons/`, которые проще править и легче держать визуально единообразными.
- Спорные действия получили разные формы: init = большой плюс, status = pulse/status, reset = undo, plan = checklist, report = document, archives = stack, no-push = local play без красного крестика.
- В верхнюю строку добавлен быстрый переключатель зарегистрированных workspace. Он читает существующий глобальный config devctl и не требует изменений ядра.

## Новое в v0.5.3

- Версия GUI поднята до `0.5.3` без изменения core-версии devctl.
- Toolbar-иконки упрощены: меньше деталей, декоративных контуров и визуального шума.
- Исправлена reset-иконка, которая могла выглядеть обрезанной в Qt.
- Кнопки, карточки и вкладки получили более мягкий borderless flat-вид.

## Новое в v0.5.4

- Версия GUI поднята до `0.5.4` без изменения core-версии devctl.
- Исправлен Qt/PySide regression в `set_running`: `workspace_combo.setEnabled(...)` теперь получает строго `bool`, а не список из global workspace registry.
- Ошибка могла появляться после успешного завершения `start`, когда patch уже применился и архив создался, но GUI пытался вернуть элементы управления в активное состояние.

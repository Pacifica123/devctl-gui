...
GUI использует JSON-режимы CLI:
...
## GUI v0.1.5 prompt-шаблон devctl-патча

В GUI добавлена кнопка `Скопировать prompt-патча`. Она кладёт в буфер обмена готовый шаблон запроса для новой ChatGPT-сессии: как собрать именно devctl-патч, а не полный архив проекта, какую структуру архива использовать, какие файлы исключать и какие проверки указывать в `manifest.json`.

Текстовый вариант шаблона также лежит в `docs/agent/devctl-patch-prompt-template.md`.

...
## GUI v0.5.1: idle-state clarity and flat icons

GUI v0.5.1 уточняет состояние по умолчанию: когда `project/`, Git, `patches/`, `archives/` и `UserTestSpace/` в порядке, а новых patch.zip нет, блок следующего действия показывает «Workspace готов, новых патчей нет» вместо жёлтого предложения повторно обновить уже актуальную структуру. Необязательные подсказки `archive.exclude` остаются в статусе, но не выглядят как проблема. Панель быстрых действий больше не зависит от системных Qt-иконок: glyph-и рисуются как собственные минималистичные flat vector icons, поэтому кнопка запуска и остальные действия выглядят единообразно в тёмной теме.


## GUI v0.5.2: SVG toolbar and quick workspace switcher

GUI v0.5.2 убирает кодовую отрисовку toolbar-глифов через `QPainter` и переносит панель быстрых действий на локальные flat SVG assets в `gui/assets/icons/`. Иконки стали плоскими, однозначнее разведены по смыслу и не зависят от системного набора Windows/Qt. В верхней строке добавлен быстрый выбор workspace из глобального devctl registry, чтобы переключаться между зарегистрированными проектами без ручного ввода пути.

## GUI v0.5.3: quieter borderless toolbar

GUI v0.5.3 делает следующий косметический проход по Qt-интерфейсу без изменения core: toolbar переводится на более спокойные SVG-глифы с меньшим числом деталей, reset-иконка получает безопасные внутренние отступы и больше не должна визуально обрезаться, а кнопки/карточки/вкладки получают более мягкий borderless flat-вид без тяжёлых контуров.

## GUI v0.5.4: running-state hotfix

GUI v0.5.4 исправляет Qt/PySide regression в состоянии завершения pipeline: `setEnabled(...)` теперь всегда получает строгий `bool`, а не список зарегистрированных workspace. Это устраняет всплывающую ошибку после успешного `devctl start`, когда архив и commit уже успевали создаться, но GUI падал на обратном включении workspace selector. Core остаётся `devctl v0.7.0`.

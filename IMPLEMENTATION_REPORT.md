...
## GUI v0.5.3 borderless minimal icon pass

Версия GUI поднята до 0.5.3. Панель быстрых действий оставлена на локальных SVG assets, но сами значки упрощены до более минималистичных line/flat-форм без декоративных окружностей и лишних внутренних деталей. Reset-иконка переработана с безопасными координатами внутри viewBox, чтобы избежать визуального обрезания в Qt. QSS также смягчён: у toolbar-кнопок, карточек, вкладок, основных кнопок, combobox/list popup и groupbox убраны тяжёлые borders, акцент сделан на спокойных фоновых поверхностях и hover-состояниях.

## GUI v0.5.4 running-state hotfix

Версия GUI поднята до 0.5.4. Исправлен регресс в `set_running`: выражение `(not running) and registered_workspaces()` в Python возвращало не `bool`, а сам список workspace, если левая часть была истинной. PySide6 требует для `QWidget.setEnabled` строгий boolean, поэтому после завершения `start` мог появляться `TypeError`, хотя сам devctl-патч уже был применён, архив создан, а обновление статуса после ошибки показывало нормальное состояние. Теперь результат явно приводится к `bool`, а базовое состояние enabled вычисляется один раз.
